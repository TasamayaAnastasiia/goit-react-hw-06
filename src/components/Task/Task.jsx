...
export const Task = ({ tasks }) => {
    ...
    const handleDelete = (e) => {
        const taskDelete = e.currentTarget.getAttribute('data-del');
        const updatedTasks = tasks.filter(task => task !== taskDelete);
        return updatedTasks;
    }
            ...
            <ul className={css.ul}>
                {tasks.map(task => task !== '' && (
                    <div className={css.li}>
                        <li key={task}>{task}</li>
                        <svg id="icon-delete" width ="32" height="32" viewBox="0 0 32 32" data-del={task} onClick={handleDelete}>
